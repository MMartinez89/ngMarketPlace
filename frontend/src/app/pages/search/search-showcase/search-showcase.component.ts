...
export class SearchShowcaseComponent implements OnInit {
  ...
  properties: Array<any> = ["name","category","sub_category","url","tags","store","title_list"];
  lisProducts: Array<any>=[];
  ...
  ngOnInit(): void {
    ...
    if(this.sort == undefined){
      this.currenRoute = `search/${this.params}`;
    }else{
      this.currenRoute = `search/${this.params}&${this.sort}`;
    }

    //FILTRAMOS LA DATA DE PRODUSCTO CON TODAS LAS PROPIEDADES 

    this.properties.forEach(property=>{
      this.productsService.getSearchData(property, this.params).subscribe((res:any)=>{
        for(let i in res){
          this.lisProducts.push(res[i]);
        }
        this.producFnc(this.lisProducts);
      });
    });
  }
