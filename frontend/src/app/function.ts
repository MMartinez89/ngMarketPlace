...
export let DinamicRating ={
  fnc: function(response:any){
    ...
    for(let i =0; i<JSON.parse(response.reviews).length; i++){
      totalReviews += Number(JSON.parse(response.reviews)[i]["review"]);
    }
    rating = Math.round(totalReviews/JSON.parse(response.reviews).length);
    return rating;
  }
}
  ...
  fnc:function(response:any){
    ...
    if(response.offer !=""){
      ...
      if(today< offerDate){
        ...
        discount = `<div class="ps-product__badge">-${value}%</div>`;
    ...
    //SI EL ARTICULO NO TIENE STOCK
    if(response.stock == 0){
      discount = `<div class="ps-product__badge out-stock">Out Of Stock</div>`;
    }

    arrayPrice[0] = price;
    arrayPrice[1] =  discount;
    return arrayPrice;
  ...
  fnc: function(){
    ...
    if(target.length>0){
      target.each(function(this:any){
        ...
        actualPages = tg.data('actual-page'),
        currentRoute = tg.data('current-route');

        tg.twbsPagination({
          totalPages: totalPages,
          startPage: actualPages,
          visiblePages: 4,
          first:'First',
          last:'Last',
        ...
        ).on("page", function(evt:any, page:any){
...
export let Tabs = {
  fnc: function(){
    $('.ps-tab-list  li > a ').on('click', function(this:any ,e:any) {
      e.preventDefault();
      var target = $(this).attr('href');
  ...
  $('.ps-tab-list.owl-slider .owl-item a').on('click', function(this:any ,e:any) {
      e.preventDefault();
      var target = $(this).attr('href');
...
export let Quantity = {
  fnc: function(){
    $(".quantity").each(function(this:any){
      var spinner = $(this), 
